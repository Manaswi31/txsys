...
void Server::initialize()
{
  ...
  passiveQueue = check_and_cast<PDropTailQueue *> (gate("in")->fromGate()->ownerModule());
...
void Server::handleMessage(cMessage *msg)
{
  if (msg == timeToXmtAPacket) { // Channel has just finished transmitting a packet.
    ...
    ASSERT(!(gate("out")->toGate()->isBusy()));
    send(msg,"out");
    // Notify ourselves the moment the transmission line finishes transmitting the packet.
    scheduleAt(gate("out")->toGate()->transmissionFinishes(), timeToXmtAPacket);
  }
}
